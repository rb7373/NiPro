    ...
    : QListWidget(parent)
{
    ...
    setMinimumSize(Constantes::tamXListaPiezas, Constantes::tamYListaPiezas);
    setMaximumSize(Constantes::tamXListaPiezas, Constantes::tamYListaPiezas);
...
void ObstaclesList::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasFormat("image/x-puzzle-piece")) {
        ...
        if (id == 0 || id == 1)
        {
            if (id == 0 && contieneFicha0 == false)
            {
                addPiece(pixmap, location, id, 0);
            }
            else if (id == 1 && contieneFicha1 == false)
            {
                addPiece(pixmap, location, id, 0);
            }
...
void ObstaclesList::startDrag(Qt::DropActions /*supportedActions*/)
{
    ...
    if (drag->exec(Qt::MoveAction) == Qt::MoveAction)
    {
        ...
        if (id == 0 || id == 1)
        {
            if (id == 0)
            {
                contieneFicha0 = false;
            }
            else if (id == 1)
            {
                contieneFicha1 = false;
            }
