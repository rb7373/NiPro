...
void ImageBuffer::addFrame(const Mat& frame)
{
    ...
    // If frame dropping is enabled,
    //do not block if buffer is full
    if(dropFrame)
    {
...
{
    qDebug() << "Tam bufer: " << getSizeOfImageBuffer();
    // Acquire semaphores
...
void ImageBuffer::clearBuffer()
{
    ...
    if(imageQueue.size()!=0)
    {
        ...
        clearBuffer1->release();
        //qDebug() << "---";
        //qDebug() << "El buffer ha sido limpiado exitosamente";
        //qDebug() << "---";
    }
    else
    {
        //qDebug() << "---";
        //qDebug() << "Advertencia: No se puede limpiar el buffer porque ya esta vacío";
        //qDebug() << "---";
    }
} // clearBuffer()
